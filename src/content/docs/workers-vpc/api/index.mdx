...
A VPC Network binding grants access to any service reachable through the bound Cloudflare Tunnel, through Cloudflare Mesh, or through a Cloudflare WAN on-ramp (GRE, IPsec, or CNI). The URL passed to `fetch()` determines the actual destination — hostname or IP address and port.
